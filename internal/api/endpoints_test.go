...
func TestSetSecrets_Success(t *testing.T) {
	...
	client := newTestClient(srv.URL, &mockAuthProvider{token: "tok", walletAddress: "w"})
	err := client.SetSecrets(context.Background(), "proj-1", "development", map[string]string{"API_KEY": "secret"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	if receivedBody["environment"] != "development" {
		t.Errorf("expected environment=development, got %v", receivedBody["environment"])
	}
}
