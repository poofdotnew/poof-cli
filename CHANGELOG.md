...
### Added
- `poof data` subcommand group — runtime reads, writes, policy queries, and atomic setMany bundles against a Poof project's data plane, all CLI-driven instead of requiring the `@pooflabs/server` SDK. Subcommands: `set`, `set-many`, `get`, `get-many`, `query`, `app-ids`. Handles the offchain (draft / Poofnet) path via SHA-256-mock signing + Poof RPC *and* the real mainnet (preview / production) path via borsh-encoded Anchor `set_documents` signing + Solana RPC (Helius default, overridable via `POOF_SOLANA_MAINNET_RPC`). The `query` subcommand accepts `--path` for simulate queries attached to specific collections (e.g. `--path "user/<addr>/BalanceCheck/any" --name simulate`).
- `poof data` shared-appid mode — all subcommands accept `--app-id <id> --chain offchain|mainnet` as an alternative to `-p <project-id> -e <env>`, so you can talk directly to a Tarobase appId (e.g. a shared primitives library someone else has deployed) without needing access to the owning Poof project. Auth is still the caller's own Solana wallet. The two modes are mutually exclusive and the CLI errors on mixed flags.
- `poof data app-ids -p <id>` — lists a project's Tarobase appIds per environment (draft / preview / production) plus the chain each maps to, so callers can grab the right `--app-id` / `--chain` pair for shared-appid mode.
